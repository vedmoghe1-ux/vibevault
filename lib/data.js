export const BUDGET_TIERS = [
  { id: "thrift", range: "₹500 – ₹2,500", name: "Broke Bestie Era", emoji: "🫰", min: 500, max: 2500 },
  { id: "main", range: "₹2,500 – ₹5,000", name: "Main Character Energy", emoji: "✨", min: 2500, max: 5000, popular: true },
  { id: "boss", range: "₹5,000 – ₹10,000", name: "Boss Era Budget", emoji: "💼", min: 5000, max: 10000 },
  { id: "nolimit", range: "₹10,000+", name: "Old Money Whisper", emoji: "👑", min: 10000, max: Infinity },
];

/* Real ₹ value of everything in an outfit — used to actually apply the
   budget tier chosen at onboarding, and by the manual price filter. */
export function outfitPriceINR(outfit) {
  return outfit.items.reduce((s, i) => s + i.price, 0) * INR_RATE;
}

export const OCCASIONS = ["Wedding", "Date Night", "Boys Hangout", "Night Out", "Everyday"];
export const SEASONS = ["Summer", "Winter", "Monsoon", "All Season"];

const SEASON_WORDS = {
  Winter: ["wool", "cashmere", "coat", "overcoat", "cardigan", "knit", "fur", "cable", "tweed", "corduroy", "sweater", "herringbone"],
  Summer: ["linen", "cotton", "shorts", "tank", "sandal", "beach", "lounge", "gingham", "straw", "mesh", "cropped tee"],
  Monsoon: ["waterproof", "rain", "windbreaker", "rubber", "ripstop", "shell"],
};
const OCCASION_WORDS = {
  Wedding: ["silk", "satin", "gown", "pearl", "velvet", "tuxedo", "cashmere overcoat"],
  "Date Night": ["lace", "satin", "heel", "bow", "cami", "slip"],
  "Boys Hangout": ["hoodie", "cargo", "sneaker", "denim", "cap", "jersey", "joggers", "graphic"],
  "Night Out": ["leather", "chain", "platform", "sequin", "harness"],
};
const AESTHETIC_DEFAULT_TAGS = {
  goth: { seasons: ["All Season"], occasions: ["Night Out"] },
  "old-money": { seasons: ["All Season"], occasions: ["Wedding", "Everyday"] },
  cottagecore: { seasons: ["Summer"], occasions: ["Everyday"] },
  "clean-girl": { seasons: ["All Season"], occasions: ["Everyday"] },
  "dark-academia": { seasons: ["Winter"], occasions: ["Everyday"] },
  y2k: { seasons: ["Summer"], occasions: ["Night Out"] },
  streetwear: { seasons: ["All Season"], occasions: ["Boys Hangout"] },
  cyberpunk: { seasons: ["All Season"], occasions: ["Night Out"] },
  coquette: { seasons: ["Summer"], occasions: ["Date Night"] },
  "mob-wife": { seasons: ["Winter"], occasions: ["Night Out"] },
  "indie-sleaze": { seasons: ["All Season"], occasions: ["Night Out"] },
  balletcore: { seasons: ["All Season"], occasions: ["Everyday"] },
  "indo-western": { seasons: ["All Season"], occasions: ["Wedding"] },
  "desi-aesthetic": { seasons: ["All Season"], occasions: ["Everyday"] },
  "new-money": { seasons: ["All Season"], occasions: ["Night Out"] },
  "thrifted-vintage": { seasons: ["All Season"], occasions: ["Everyday"] },
};

/* Reads the actual pieces in an outfit and tags it by season/occasion —
   falls back to a sensible per-aesthetic default when nothing in the
   item names gives a strong enough signal either way. */
export function tagOutfit(outfit) {
  const text = (outfit.items.map((i) => `${i.name} ${i.brand}`).join(" ") + " " + (outfit.blurb || "")).toLowerCase();
  const matchAny = (dict) => Object.entries(dict).filter(([, words]) => words.some((w) => text.includes(w))).map(([k]) => k);

  let seasons = matchAny(SEASON_WORDS);
  let occasions = matchAny(OCCASION_WORDS);
  const fallback = AESTHETIC_DEFAULT_TAGS[outfit.aesthetic] ?? { seasons: ["All Season"], occasions: ["Everyday"] };
  if (!seasons.length) seasons = fallback.seasons;
  if (!occasions.length) occasions = fallback.occasions;
  return { seasons: [...new Set(seasons)], occasions: [...new Set(occasions)] };
}

export const AESTHETICS = [
  { id: "goth", name: "Goth", tagline: "Ritual black, silver hardware, nothing soft", a1: "#B892FF", a2: "#FF4D6D", tone: "#151021", note: "lace · leather · chrome", promise: "for quiet menace", image: "/products/cat-goth.jpg" },
  { id: "old-money", name: "Old Money", tagline: "Quiet cloth that never mentions the price", a1: "#D9C89A", a2: "#8FB09B", tone: "#171A16", note: "cashmere · poplin · gold", promise: "for confidence", image: "/products/cat-old-money.jpg" },
  ...
  { id: "clean-girl", name: "Clean Girl", tagline: "Slicked back, unbothered, tonal head to toe", a1: "#E6D8C3", a2: "#9CC7D8", tone: "#14151A", note: "jersey · gold · glass", promise: "for effortless control", image: "/products/cat-clean-girl.jpg" },
  { id: "dark-academia", name: "Dark Academia", tagline: "Library dust, tweed and long arguments", a1: "#C9A27E", a2: "#7E96C4", tone: "#161318", note: "tweed · wool · leather", promise: "for intellectual mystery", image: "/products/cat-dark-academia.jpg" },
  { id: "y2k", name: "Y2K", tagline: "Low rise, high shine, zero restraint", a1: "#8FE9FF", a2: "#FF7BD5", tone: "#101827", note: "chrome · denim · gloss", promise: "for chaotic confidence", image: "/products/cat-y2k.jpg" },
  { id: "streetwear", name: "Streetwear", tagline: "Volume, layers, and shoes that argue back", a1: "#F5E663", a2: "#FF8A3D", tone: "#141414", note: "nylon · fleece · rubber", promise: "for street credibility", image: "/products/cat-streetwear.jpg" },
  ...
  { id: "indo-western", name: "Indo Western", tagline: "Where a kurta meets a blazer and neither apologizes", a1: "#E8A33D", a2: "#8B3A62", tone: "#1A1410", note: "kurta · bandhgala · draped", promise: "for festival main character energy", image: "/products/cat-indo-western.jpg" },
  { id: "desi-aesthetic", name: "Desi Aesthetic", tagline: "Everyday India, dressed with intention", a1: "#D9663B", a2: "#4C7A6B", tone: "#171310", note: "cotton · block print · juttis", promise: "for rooted confidence", image: "/products/cat-desi-aesthetic.jpg" },
  { id: "new-money", name: "New Money", tagline: "Logos loud, bags heavier, subtlety optional", a1: "#F2C230", a2: "#1F6FEB", tone: "#141014", note: "monogram · leather · chrome", promise: "for main character flex", image: "/products/cat-new-money.jpg" },
  { id: "thrifted-vintage", name: "Thrifted Vintage", tagline: "Someone else's closet, now it's yours", a1: "#C97B4A", a2: "#7C8B6F", tone: "#191510", note: "denim · corduroy · secondhand", promise: "for one-of-one energy" },
];

export const shopUrl = (brand, name) =>
  `https://www.google.com/search?q=${encodeURIComponent(`${brand} ${name}`)}&tbm=shop`;

export const mk = (id, slot, name, brand, price, tone, image) => ({
  id, slot, name, brand, price, tone, image, url: shopUrl(brand, name),
});
...
/* For real products sourced from an Indian retailer (e.g. Myntra via an
   ...
   `image` is optional — paste the product photo URL from the retailer's
   own page (right-click → Copy image address) and it renders as a real
   photo everywhere a swatch would otherwise show. */
const INR_RATE = 83.1;
export const mkReal = (id, slot, name, brand, priceINR, tone, description, url, image) => ({
  id, slot, name, brand, price: priceINR / INR_RATE, tone, description, url, image, real: true,
});
...
export const OUTFITS = [
  ...
  { id: "o24", aesthetic: "old-money", title: "Upper East Side", curator: "@you", saves: 0, promoted: false, real: true,
    image: "/products/old-money-cover.jpg",
    hotspots: { Top: [50, 32], Bottoms: [48, 68], Shoes: [46, 94] },
    blurb: "Resort tailoring that never tries too hard — texture and fit doing the work logos usually do.",
    items: [
      mkReal("i98", "Top", "Men Knitted Fit Textured Spread Collar Casual Shirt", "RAHI TRENDZ", 413, "#D8C7A8", "Resort vibe", "https://myntr.it/572d4HD", "/products/upper-east-side-shirt.jpg"),
      mkReal("i99", "Bottoms", "Men Cotton Comfort Beach Lounge Pants", "The Veshti Company", 1999, "#C9B89A", "Stealth wealth", "https://myntr.it/x5XAkOc", "/products/upper-east-side-pants.jpg"),
      mkReal("i100", "Shoes", "Men JOAN Beige All Day Sneakers", "U.S. Polo Assn.", 2621, "#E3D5BE", "Retro tennis", "https://myntr.it/vU1232y", "/products/upper-east-side-sneakers.jpg"),
    ] },

  { id: "o25", aesthetic: "streetwear", title: "Acobi After Hours", curator: "@you", saves: 0, promoted: false, real: true,
    image: "/products/streetwear-cover.jpg",
    hotspots: { Top: [50, 30], Bottoms: [50, 62], Shoes: [50, 93] },
    blurb: "Loud tee, loose pants, clean kicks — dressed for a room that's about to get crowded.",
    items: [
      mkReal("i101", "Top", "F.R.I.E.N.D.S Men Printed Round Neck Pure Cotton Black T-Shirt", "Souled Store", 1048, "#141414", "Nostalgic graphic pop culture flex", "https://fktr.in/TgCvNxQ", "/products/streetwear-shirt.jpg"),
      mkReal("i102", "Bottoms", "Men Loose Fit Black Cotton Blend Trousers", "Trackflip", 340, "#161616", "Lightning statement baggy silhouette", "https://fktr.in/EfyediW", "/products/streetwear-bottoms.jpg"),
      mkReal("i103", "Shoes", "Revolution 7 Men's Road Running Shoes", "Nike", 3326, "#2A2D31", "Minimal clean monochromatic kicks", "https://myntr.it/nRDQRHN", "/products/streetwear-shoes.jpg"),
    ] },

  { id: "o26", aesthetic: "new-money", title: "Downtown Luxe", curator: "@you", saves: 0, promoted: false, real: true,
    image: "/products/downtown-luxe-cover.jpg",
    hotspots: { Top: [50, 30], Bottoms: [50, 65], Shoes: [50, 93] },
    blurb: "Striped knit, snap-side track pants, high-heat kicks — money that wants to be seen from across the rooftop.",
    items: [
      mkReal("i104", "Top", "Men T-shirt", "Nike", 4995, "#B08B6A", "Premium neutral warmth", "https://myntr.it/LlZOjYN"),
      mkReal("i105", "Bottoms", "Men Adibreak Classic Tracksuit Bottoms", "ADIDAS Originals", 7599, "#4A3B2E", "Iconic snap-side flex", "https://myntr.it/ZW9HT17"),
      mkReal("i106", "Shoes", "Air Jordan 1 Mid SE Men Mid-Top Shoes", "Nike", 12295, "#8A6A4E", "High-heat status kicks", "https://myntr.it/iN3A89E"),
    ] },
];
